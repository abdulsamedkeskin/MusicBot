...
async def on_message(message):
    if message.author.bot == False:
        ...
        if ctx == "clear":
            embed = discord.Embed(
                description="Mesajlar Siliniyor...", color=0x3498db)
            await message.channel.send(embed=embed)
            await message.channel.purge(limit=None)
        if ctx == "help":
            embed = discord.Embed(
                title="Yardım", description="**Bütün Kodlar**", color=0x3498db)
        ...
        if ctx.split()[0] == "play":
            if message.author.voice and message.author.voice.channel:
                if ctx.find("play ") != -1:
                    embed = discord.Embed(
                        description="**Şarkı Aranıyor...**", color=0x00ff00)
                    ...
                    if selection_check == "1\uFE0F\u20E3":
                        ...
                        voice_channel.play(
                            discord.FFmpegPCMAudio(
                                source=source, **FFMPEG_OPTIONS))
                        embed = discord.Embed(
                            title="Şu anda oynatılan şarkı",
                    ...
                    if selection_check == "2\uFE0F\u20E3":
                        ...
                        embed = discord.Embed(
                            title="Şu anda oynatılan şarkı",
                            description=f"{results[1]['title']}",
                            color=0x00ff00).set_thumbnail(
                                url=results[1]['thumbnail'])
                        await message.channel.send(embed=embed)
                    if selection_check == "3\uFE0F\u20E3":
                        ...
                        embed = discord.Embed(
                            title="Şu anda oynatılan şarkı",
                            description=f"{results[2]['title']}",
                            color=0x00ff00).set_thumbnail(
                                url=results[2]['thumbnail'])
                        await message.channel.send(embed=embed)
                    if selection_check == "4\uFE0F\u20E3":
                        ...
                        embed = discord.Embed(
                            title="Şu anda oynatılan şarkı",
                            description=f"{results[3]['title']}",
                            color=0x00ff00).set_thumbnail(
                                url=results[3]['thumbnail'])
                        await message.channel.send(embed=embed)
                    if selection_check == "5\uFE0F\u20E3":
                        ...
                        embed = discord.Embed(
                            title="Şu anda oynatılan şarkı",
                            description=f"{results[4]['title']}",
                            color=0x00ff00).set_thumbnail(
                                url=results[4]['thumbnail'])
                        await message.channel.send(embed=embed)
                    ...
                    embed = discord.Embed(
                        description="Lütfen şarkının ismini de girin",
                        color=0x00ff00)
                    await message.channel.send(embed=embed)
            else:
                embed = discord.Embed(
        ...
        if ctx == "pause":
            ...
            if voice and voice.is_playing():
                voice.pause()
                embed = discord.Embed(
                    description="Şarkı duraklatıldı.", color=0x00ff00)
                await message.channel.send(embed=embed)
            else:
                embed = discord.Embed(
                    description="Şarkıl duraklatılamadı.", color=0x00ff00)
                await message.channel.send(embed=embed)
        if ctx == "resume":
            voice = get(client.voice_clients, guild=message.guild)
            if voice and voice.is_paused():
                voice.resume()
                embed = discord.Embed(
                    description="Şarkı devam ettiriliyor.", color=0x00ff00)
                await message.channel.send(embed=embed)
            else:
                embed = discord.Embed(
                    description="Şarkı devam ettirilemiyor", color=0x00ff00)
                await message.channel.send(embed=embed)
        if ctx == "stop":
            voice = get(client.voice_clients, guild=message.guild)
            if voice and voice.is_playing():
                voice.stop()
                embed = discord.Embed(
                    description="Şarkı durduruldu.", color=0x00ff00)
                await message.channel.send(embed=embed)
            else:
                embed = discord.Embed(
                    description="Şarkı durdurulamadı.", color=0x00ff00)
                await message.channel.send(embed=embed)
        if ctx == "destroy":
            voice = get(client.voice_clients, guild=message.guild)
            if voice:
                ...
                embed = discord.Embed(
                    description="Bir hata meydana geldi", color=0x00ff00)
                await message.channel.send(embed=embed)
